...
/**
 * Multi select datamodel to handle multi selection Only accepts data
 * implementing java.util.List for now, a jsf component handling it would be
 * more generic.
 *
...
public class SelectDataModelImpl extends DataModel implements SelectDataModel {
    ...
    public SelectDataModelListener[] getSelectModelListeners() {
        ...
        return selectListeners.toArray(new SelectDataModelListener[selectListeners.size()]);
    }

    public List<SelectDataModelRow> getRows() {
        // XXX AT: sort rows by title until sort is fixed, see NXP-528
        /*
         * XXX NP : This sorts by default the list, and for now I don't need it
         * to be sorted if (rows != null) { Collections.sort(rows, new
         * SelectDocumentDataModelRowComparator( "dublincore", "title", true));
         * }
         */
        return rows;
    }
