...
import org.nuxeo.ecm.platform.ui.web.component.UINXForm;
...
/**
 * Overrides standard form renderer to detect forms inclusion and replace them
 * by div elements that can be turned into forms again. This is needed for
 * forms inclusion via fancybox include elements.
 *
 * @since 5.6
 */
public class NXFormRenderer extends FormRenderer {
    ...
    public void encodeBegin(FacesContext context, UIComponent component)
        ...
        writer.write('\n');
        boolean isNested = isNestedInForm(component, context);
        if (isNested) {
            // change client id to be a container
        ...
        if (isNested) {
            if (styleClass == null) {
                styleClass = "nxNestedForm";
            } else {
                styleClass = "nxNestedForm " + styleClass;
            }
        }
        if (styleClass != null) {
    ...
    protected boolean isNestedInForm(UIComponent uiComponent,
            FacesContext facesContext) {
        if (uiComponent instanceof UINXForm) {
            return ((UINXForm) uiComponent).isNestedInForm(facesContext);
        }
        return false;
    }
